...
**TUN/Fake-IP is direct at the HTTP layer but synthetic at DNS.** A TUN proxy can leave the operating-system HTTP proxy disabled while answering public hostnames from a private or benchmark-only range; the connection is intercepted below Undici. Treating that answer as an ordinary private destination breaks public fetches, while globally classifying a range as public would let IP-literal URLs bypass SSRF policy. `web-fetch-http.verifySyntheticDns` therefore activates an independent DNS-over-HTTPS fallback only after system DNS produces a non-public hostname answer. The provider discards that local answer and pins the request to independently verified public IPv4/IPv6 addresses. Custom ranges and runtime network changes need no Host restart, while private hostnames, non-public verification answers, and non-public IP literals remain blocked. `syntheticDnsRanges` remains available for deployments that explicitly trust a local resolver's configured transport ranges.
