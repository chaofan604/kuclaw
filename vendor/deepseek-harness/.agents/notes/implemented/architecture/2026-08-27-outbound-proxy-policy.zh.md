...
**TUN/Fake-IP 在 HTTP 层是直连，在 DNS 层却是合成地址。** TUN 代理可以让操作系统 HTTP 代理保持关闭，同时用私网或仅用于基准测试的地址段回答公共主机名；连接会在 Undici 下层被截获。把这种回答当作普通私网目标会让公共抓取失败，而把整个地址段全局归类为公网又会让 IP 字面量 URL 绕过 SSRF 策略。因此 `web-fetch-http.verifySyntheticDns` 只在系统 DNS 为主机名返回非公开地址后触发独立的 DNS-over-HTTPS 回退。提供方会丢弃本地回答，把请求固定到独立验证过的公网 IPv4/IPv6 地址。自定义地址段和运行期网络切换不需要重启 Host；私有主机名、验证所得的非公开地址以及非公开 IP 字面量仍会被阻止。`syntheticDnsRanges` 继续保留给显式信任本地解析器传输地址段的部署。
