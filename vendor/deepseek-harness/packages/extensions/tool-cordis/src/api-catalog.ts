...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'scheduledTaskController',
    summary: 'Host owner of the `scheduledTasks` Remote namespace.',
    description: 'Host owner of the `scheduledTasks` Remote namespace.',
    methods: [
      {
        signature: '@Remote list(): ScheduledTask[]',
        description: 'Return every persistent task.',
        parameters: [],
        returns: 'the stored tasks, earliest next occurrence first.',
      },
      {
        signature: '@Remote create(input: ScheduledTaskInput): ScheduledTask',
        description: 'Create one task.',
        parameters: [{ name: 'input', description: 'user-authored fields; invalid values reject with the service error.' }],
        returns: 'the stored task with its computed next occurrence.',
      },
      {
        signature: '@Remote update(input: ScheduledTaskUpdate): ScheduledTask',
        description: 'Update one task.',
        parameters: [{ name: 'input', description: 'task id plus the fields to replace.' }],
        returns: 'the stored task with its re-derived next occurrence.',
      },
      {
        signature: '@Remote remove(id: ScheduledTaskId): void',
        description: 'Delete one idle task.',
        parameters: [{ name: 'id', description: 'task identity; a task with an active run rejects.' }],
      },
      {
        signature: '@Remote setEnabled(id: ScheduledTaskId, enabled: boolean): ScheduledTask',
        description: 'Enable or pause one task.',
        parameters: [{ name: 'id', description: 'task identity.' }, { name: 'enabled', description: 'whether the task may fire.' }],
        returns: 'the stored task after the change.',
      },
      {
        signature: '@Remote runNow(id: ScheduledTaskId): ScheduledTaskRun',
        description: 'Start one independent execution immediately.',
        parameters: [{ name: 'id', description: 'task identity.' }],
        returns: 'the admitted run, already `skipped` when the task is active under `skip`.',
      },
      {
        signature: '@Remote cancelRun(id: ScheduledTaskRunId): Promise<void>',
        description: 'Cancel one live execution.',
        parameters: [{ name: 'id', description: 'run identity; a settled run is a no-op.' }],
      },
      {
        signature: '@Remote runs(taskId: ScheduledTaskId, limit?: number): ScheduledTaskRun[]',
        description: 'Return bounded run history for one task.',
        parameters: [{ name: 'taskId', description: 'task identity.' }, { name: 'limit', description: 'maximum rows, bounded to 1..200; defaults to 50.' }],
  ...
  {
    key: 'scheduledTasks',
    summary: 'SQLite-backed Host service for App-authored recurring Agent tasks.',
    description: 'SQLite-backed Host service for App-authored recurring Agent tasks.',
    methods: [
      {
        signature: 'list(): ScheduledTask[]',
        description: 'List tasks ordered by next occurrence.',
        parameters: [],
        returns: 'every stored task, earliest next occurrence first.',
      },
      {
        signature: 'create(input: ScheduledTaskInput): ScheduledTask',
        description: 'Create one persistent task and arm its first occurrence.',
        parameters: [{ name: 'input', description: 'user-authored fields; invalid rules, zones, or required fields throw.' }],
        returns: 'the stored task with its computed next occurrence.',
      },
      {
        signature: 'update(update: ScheduledTaskUpdate): ScheduledTask',
        description: 'Update one task without changing its identity or creation time.',
        parameters: [{ name: 'update', description: 'task id plus the fields to replace.' }],
        returns: 'the stored task with its re-derived next occurrence.',
      },
      {
        signature: 'setEnabled(id: ScheduledTaskId, enabled: boolean): ScheduledTask',
        description: 'Enable or pause one task.',
        parameters: [{ name: 'id', description: 'task identity.' }, { name: 'enabled', description: 'whether the task may fire.' }],
        returns: 'the stored task after the change.',
      },
      {
        signature: 'remove(id: ScheduledTaskId): void',
        description: 'Delete one idle task and its retained run history.',
        parameters: [{ name: 'id', description: 'task identity; a task with an active run is refused.' }],
      },
      {
        signature: 'runNow(id: ScheduledTaskId): ScheduledTaskRun',
        description: 'Start one independent run immediately without moving its Cron anchor.',
        parameters: [{ name: 'id', description: 'task identity.' }],
        returns: 'the admitted run, already `skipped` when the task is active under `skip`.',
      },
      {
        signature: 'cancelRun(id: ScheduledTaskRunId): Promise<void>',
        description: 'Cancel one running scheduled execution.',
        parameters: [{ name: 'id', description: 'run identity; a settled run is a no-op.' }],
      },
      {
        signature: 'listRuns(taskId: ScheduledTaskId, limit: number = 50): ScheduledTaskRun[]',
        description: 'List retained run history newest first.',
        parameters: [{ name: 'taskId', description: 'task identity.' }, { name: 'limit', description: 'maximum rows, bounded to 1..200; defaults to 50.' }],
        returns: 'the task\'s runs, newest scheduled first.',
      },
      {
        signature: 'onChanged(listener: ChangedListener): () => void',
        description: 'Observe committed task or run changes.',
        parameters: [{ name: 'listener', description: 'called after a task or run row is committed.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'ScheduledTask',
    declaration: 'export interface ScheduledTask {\n    readonly id: ScheduledTaskId;\n    readonly name: string;\n    readonly workspacePath: string;\n    readonly cron: string;\n    readonly timeZone: string;\n    readonly prompt: string;\n    readonly model: ScheduledTaskModel;\n    readonly preset: string;\n    readonly enabled: boolean;\n    readonly concurrency: ScheduledTaskConcurrency;\n    readonly missedRunPolicy: ScheduledTaskMissedRunPolicy;\n    readonly nextRunAt: number;\n    readonly createdAt: number;\n    readonly updatedAt: number;\n}',
  },
  {
    name: 'ScheduledTaskConcurrency',
    declaration: 'export type ScheduledTaskConcurrency = \'skip\' | \'queue\';',
  },
  {
    name: 'ScheduledTaskId',
    declaration: 'export type ScheduledTaskId = Branded<\'scheduled-task-id\'>;',
  },
  {
    name: 'ScheduledTaskInput',
    declaration: 'export interface ScheduledTaskInput {\n    readonly name: string;\n    readonly workspacePath: string;\n    readonly cron: string;\n    readonly timeZone: string;\n    readonly prompt: string;\n    readonly model: ScheduledTaskModel;\n    readonly preset?: string;\n    readonly enabled?: boolean;\n    readonly concurrency?: ScheduledTaskConcurrency;\n    readonly missedRunPolicy?: ScheduledTaskMissedRunPolicy;\n}',
  },
  {
    name: 'ScheduledTaskMissedRunPolicy',
    declaration: 'export type ScheduledTaskMissedRunPolicy = \'skip\' | \'run-once\';',
  },
  {
    name: 'ScheduledTaskModel',
    declaration: 'export interface ScheduledTaskModel {\n    readonly provider: string;\n    readonly model: string;\n    readonly reasoningEffort?: string;\n}',
  },
  {
    name: 'ScheduledTaskRun',
    declaration: 'export interface ScheduledTaskRun {\n    readonly id: ScheduledTaskRunId;\n    readonly taskId: ScheduledTaskId;\n    readonly scheduledAt: number;\n    readonly sessionId?: string;\n    readonly status: ScheduledTaskRunStatus;\n    readonly startedAt?: number;\n    readonly finishedAt?: number;\n    readonly errorCode?: string;\n    readonly errorMessage?: string;\n}',
  },
  {
    name: 'ScheduledTaskRunId',
    declaration: 'export type ScheduledTaskRunId = Branded<\'scheduled-task-run-id\'>;',
  },
  {
    name: 'ScheduledTaskRunStatus',
    declaration: 'export type ScheduledTaskRunStatus = \'pending\' | \'running\' | \'succeeded\' | \'failed\' | \'cancelled\' | \'timed-out\' | \'skipped\';',
  },
  {
    name: 'ScheduledTaskUpdate',
    declaration: 'export interface ScheduledTaskUpdate extends Partial<ScheduledTaskInput> {\n    readonly id: ScheduledTaskId;\n}',
  },
  {
