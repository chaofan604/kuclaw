...
| `syntheticDnsRanges` | `[]` | 本地 TUN/Fake-IP 解析器仅用于主机名传输地址的可信 CIDR |
| `verifySyntheticDns` | `false` | 用独立公共 DNS 地址替换系统 DNS 返回的非公开地址 |
...
提供方保持请求匿名且有界：只接受不含内嵌凭据且不超过 2,048 个字符的 `http:` 与 `https:` URL。它只解析一次主机名；只要结果中有任何 IPv4 或 IPv6 地址既不是公共单播地址，也不属于显式配置的合成 DNS 传输地址段，就拒绝整个结果，并把连接固定到已校验的地址集合。启用 `verifySyntheticDns` 后，系统返回非公开地址时会触发独立的 DNS-over-HTTPS 复核；提供方会丢弃本地 Fake-IP，并把请求固定到复核得到的真实公网 IPv4/IPv6 地址。这条路径不依赖已知 Fake-IP CIDR，每次受影响的解析都会执行，因此自定义地址段和运行期网络切换都不需要重启 Host。公共 DNS 中不存在的主机名、公共 DNS 返回的任何非公开地址，以及所有非公开 IP 字面量仍会被阻止。IPv6 检查会发现活动 DNS64 前缀，并拒绝指向非公开 IPv4 的转换地址。每次同源重定向都会重复解析与固定；跨源重定向会失败并要求重新调用。提供方还强制执行字节、字符、跳数和时间上限，拒绝不支持的内容类型，并发送显式产品 `User-Agent`。
