...
import { createPublicAddressResolver, resolveViaPublicDns } from './network.ts'
...
export interface Config {
  ...
  userAgent?: string
  /** Trusted CIDR ranges used only as synthetic DNS transport addresses for hostnames. */
  syntheticDnsRanges?: string[]
  /** Verify non-public system DNS answers through public DNS and pin the verified public addresses. */
  verifySyntheticDns?: boolean
}
...
export const Config: z<Config> = z.object({
  ...
  syntheticDnsRanges: z.array(z.string()).default([]),
  verifySyntheticDns: z.boolean().default(false),
})
...
export function apply(ctx: Context, config: Config): void {
  ...
  const resolveAddresses = resolved.syntheticDnsRanges.length === 0 && !resolved.verifySyntheticDns
    ? undefined
    : createPublicAddressResolver(
      resolved.syntheticDnsRanges,
      undefined,
      resolved.verifySyntheticDns ? resolveViaPublicDns : undefined,
    )
