...
/** Independent public-DNS resolver used only when the operating system returns non-public addresses. */
export type VerifiedPublicResolver = (hostname: string, signal: AbortSignal) => Promise<LookupAddress[]>
...
const PUBLIC_DNS_MAX_RESPONSE_BYTES = 262_144
...
export async function resolvePublicAddresses(
  ...
  return resolveAllowedAddresses(hostname, signal, resolver, [])
}

/**
 * Build a resolver that also accepts configured synthetic DNS transport ranges for hostnames.
 *
 * @param ranges - CIDR ranges a trusted local TUN resolver uses as synthetic transport addresses.
 * @param resolver - lookup implementation, overridden only by focused tests.
 * @returns a resolver that keeps the normal public-address policy plus those hostname-only ranges.
 */
export function createPublicAddressResolver(
  ranges: readonly string[],
  resolver: AddressResolver = systemLookup,
  verifiedPublicResolver?: VerifiedPublicResolver,
): (hostname: string, signal: AbortSignal) => Promise<PublicAddress[]> {
  const parsed = ranges.map((range) => {
    try {
      return ipaddr.parseCIDR(range)
    } catch {
      throw new Error(`web-fetch-http: syntheticDnsRanges contains invalid CIDR ${JSON.stringify(range)}`)
    }
  })
  return (hostname, signal) => resolveAllowedAddresses(
    hostname,
    signal,
    resolver,
    parsed,
...
async function resolveAllowedAddresses(
  ...
  syntheticDnsRanges: readonly ParsedCidr[],
  verifiedPublicResolver?: VerifiedPublicResolver,
): Promise<PublicAddress[]> {
  ...
  if (blocked && literalFamily === 0 && verifiedPublicResolver !== undefined) {
    const verified = await raceWithSignal(verifiedPublicResolver(unbracketed, signal), signal)
    if (verified.length > 0) {
      validateAddressEntries(hostname, verified)
    }
    if (verified.length > 0 && verified.every(entry => isPublicIpAddress(entry.address))) {
      return verified.map(entry => ({ address: entry.address, family: entry.family as 4 | 6 }))
    }
  }

  const hasIpv6 = resolved.some(entry => entry.family === 6)
  ...
  for (const entry of resolved) {
    const syntheticDnsAddress = literalFamily === 0 && addressMatchesRanges(entry.address, syntheticDnsRanges)
    if (!isPublicIpAddress(entry.address) && !syntheticDnsAddress) {
...
function validateAddressEntries(hostname: string, entries: readonly LookupAddress[]): void {
  if (entries.length === 0) {
    throw new WebError(`hostname "${hostname}" resolved to no addresses`, 'WEB_PROVIDER_ERROR')
  }
  if (entries.some(entry => (entry.family !== 4 && entry.family !== 6) || isIP(entry.address) !== entry.family)) {
    throw new WebError(`hostname "${hostname}" resolved to an invalid IP address`, 'WEB_PROVIDER_ERROR')
  }
}

/** Resolve a hostname through independent HTTPS resolvers and keep only verified public addresses. */
export async function resolveViaPublicDns(hostname: string, signal: AbortSignal): Promise<LookupAddress[]> {
  let lastError: unknown
  let receivedValidResponse = false
  for (const endpoint of PUBLIC_DNS_ENDPOINTS) {
    const answers = await Promise.allSettled([
      queryPublicDns(endpoint, hostname, 'A', signal),
      queryPublicDns(endpoint, hostname, 'AAAA', signal),
    ])
    const unique = new Map<string, LookupAddress>()
    for (const answer of answers) {
      if (answer.status === 'rejected') {
        lastError = answer.reason
        continue
      }
      receivedValidResponse = true
      for (const entry of answer.value) {
        if (!isPublicIpAddress(entry.address)) continue
        unique.set(`${String(entry.family)}:${entry.address}`, entry)
      }
    }
    if (unique.size > 0) return [...unique.values()]
  }
  if (signal.aborted) throw new Error('public DNS verification aborted', { cause: signal.reason })
  if (!receivedValidResponse && lastError !== undefined) {
    throw new WebError('public DNS verification failed', 'WEB_PROVIDER_ERROR', { cause: lastError })
  }
  return []
}

async function queryPublicDns(
  endpoint: string,
  hostname: string,
  recordType: 'A' | 'AAAA',
  signal: AbortSignal,
  ...
  if (!response.ok) {
    ...
    throw new Error(`public DNS returned HTTP ${String(response.status)}`)
  }
  const declaredLength = Number(response.headers.get('content-length'))
  if (Number.isFinite(declaredLength) && declaredLength > PUBLIC_DNS_MAX_RESPONSE_BYTES) {
    await response.body?.cancel()
    throw new Error('public DNS response is too large')
  }
  const text = await response.text()
  if (Buffer.byteLength(text) > PUBLIC_DNS_MAX_RESPONSE_BYTES) {
    throw new Error('public DNS response is too large')
  }
  const body: unknown = JSON.parse(text)
  if (!isDnsJsonResponse(body)) throw new Error('public DNS returned an invalid response')
  const family = recordType === 'A' ? 4 : 6
  return (body.Answer ?? [])
    .filter(answer => answer.type === (recordType === 'A' ? 1 : 28) && isIP(answer.data) === family)
    .map(answer => ({ address: answer.data, family }))
}
...
function addressMatchesRanges(input: string, ranges: readonly ParsedCidr[]): boolean {
  let address: ipaddr.IPv4 | ipaddr.IPv6
  try {
    address = ipaddr.parse(input)
  } catch {
    return false
  }
  return ranges.some(([network, prefix]) => address.kind() === network.kind() && address.match(network, prefix))
}
