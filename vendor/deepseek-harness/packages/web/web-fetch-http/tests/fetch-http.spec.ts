...
describe('public-network policy', () => {
  ...
  it('accepts configured synthetic DNS ranges for hostnames but never for IP literals', async () => {
    const resolver = createPublicAddressResolver(
      ['198.18.0.0/15'],
      async () => [{ address: '198.18.0.132', family: 4 }],
    )
    await expect(resolver('example.test', new AbortController().signal))
      .resolves.toEqual([{ address: '198.18.0.132', family: 4 }])
    await expect(resolver('198.18.0.132', new AbortController().signal))
      .rejects.toThrow(expect.objectContaining({ code: 'WEB_BLOCKED_URL' }))
  })

  it('keeps other private answers blocked when a synthetic DNS range is configured', async () => {
    const resolver = createPublicAddressResolver(
      ['198.18.0.0/15'],
      async () => [{ address: '10.0.0.8', family: 4 }],
    )
    await expect(resolver('private.test', new AbortController().signal))
      .rejects.toThrow(expect.objectContaining({ code: 'WEB_BLOCKED_URL' }))
  })

  it('replaces arbitrary IPv4 and IPv6 Fake-IP answers with independently verified public addresses', async () => {
    const systemResolver = vi.fn(async () => [
      { address: '10.64.0.9', family: 4 },
      { address: 'fd00::9', family: 6 },
    ])
    const verifiedResolver = vi.fn(async () => [
      { address: '93.184.216.34', family: 4 },
      { address: '2606:2800:220:1:248:1893:25c8:1946', family: 6 },
    ])
    const resolver = createPublicAddressResolver([], systemResolver, verifiedResolver)

    await expect(resolver('example.com', new AbortController().signal)).resolves.toEqual([
      { address: '93.184.216.34', family: 4 },
      { address: '2606:2800:220:1:248:1893:25c8:1946', family: 6 },
    ])
    expect(verifiedResolver).toHaveBeenCalledWith('example.com', expect.any(AbortSignal))
  })

  it('keeps real private DNS answers blocked when public DNS cannot verify the hostname', async () => {
    const verifiedResolver = vi.fn(async () => [])
    const resolver = createPublicAddressResolver(
      [],
      async () => [{ address: '10.0.0.8', family: 4 }],
    ...
    await expect(resolver('intranet.example', new AbortController().signal))
      .rejects.toThrow(expect.objectContaining({ code: 'WEB_BLOCKED_URL' }))
  })

  it('never public-DNS-verifies a non-public IP literal', async () => {
    const verifiedResolver = vi.fn(async () => [{ address: '8.8.8.8', family: 4 }])
    const resolver = createPublicAddressResolver([], async () => [], verifiedResolver)

    await expect(resolver('10.0.0.8', new AbortController().signal))
      .rejects.toThrow(expect.objectContaining({ code: 'WEB_BLOCKED_URL' }))
    expect(verifiedResolver).not.toHaveBeenCalled()
  })

  it('rejects an invalid synthetic DNS CIDR at configuration time', () => {
    expect(() => createPublicAddressResolver(['not-a-cidr'])).toThrow('invalid CIDR')
  })
