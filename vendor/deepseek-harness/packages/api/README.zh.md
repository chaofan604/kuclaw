...
| [`scheduled-task-controller/`](scheduled-task-controller/README.zh.md) | 把 Host 的持久化定时任务暴露为 Remote 的 CRUD、启用/暂停、立即运行、取消与有界历史调用。 | `ctx.scheduledTaskController` / `ctx.remote.scheduledTasks` |
